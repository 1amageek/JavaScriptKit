...
build_mode() {
  ...
  if [ "$mode" = "embedded" ]; then
    env JAVASCRIPTKIT_EXPERIMENTAL_EMBEDDED_WASM=1 "$swift_bin" package \
      --build-system native \
...
echo "Building standard WASM with $standard_sdk"
build_mode "standard" "$standard_sdk"

echo "Building Embedded WASM with $embedded_sdk"
build_mode "embedded" "$embedded_sdk"

standard_wasm="$output_dir/standard/Package/EmbeddedApp.wasm"
embedded_wasm="$output_dir/embedded/Package/EmbeddedApp.wasm"
